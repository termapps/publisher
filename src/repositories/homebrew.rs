    ...
    fn publish(&self, info: &PublishInfo, version: &str, dry_run: bool) -> Result {
        ...
        write_and_add(&sh, &dir, format!("Formula/{name}.rb"), || {
            vec![
                ...
                format!("      url \"https://github.com/{repository}/releases/download/v#{{version}}/{cli_name}-#{{version}}-aarch64-apple-darwin.zip\""),
                format!("      sha256 {:?}", checksums.get(&Target::Aarch64AppleDarwin).unwrap()),
                format!("    else"),
                format!("      url \"https://github.com/{repository}/releases/download/v#{{version}}/{cli_name}-#{{version}}-x86_64-apple-darwin.zip\""),
                format!("      sha256 {:?}", checksums.get(&Target::X86_64AppleDarwin).unwrap()),
                format!("    end"),
                format!("  elsif OS.linux?"),
                format!("     url \"https://github.com/{repository}/releases/download/v#{{version}}/{cli_name}-#{{version}}-x86_64-unknown-linux-gnu.zip\""),
